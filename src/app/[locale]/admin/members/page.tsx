...
export default function MembersPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border capitalize ${styles[status] || styles.inactive}`}>
        {status}
      </span>
    );
  };
  ...
  const columns: Column<Member>[] = [
    ...
    { key: 'phone', header: 'Phone', render: (member: Member) => member.phone || '-' },
    {
      key: 'serviceType',
      header: 'Service',
      render: (member: Member) => <span className="text-white/80">{member.serviceType || '-'}</span>,
    },
    {
      key: 'membershipTier',
      header: 'Tier',
      render: (member: Member) => {
        ...
        return (
          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border capitalize ${tierColors[tierKey] || tierColors.silver}`}>
            {member.membershipTier}
          </span>
        );
      },
    },
    { key: 'status', header: 'Status', render: (member: Member) => getStatusBadge(member.status) },
    {
      key: 'endDate',
      header: 'Expires',
      render: (member: Member) => member.endDate ? new Date(member.endDate).toLocaleDateString('en-GB') : '-',
    },
    {
      key: 'actions',
      header: '',
      className: 'w-12',
      render: () => <span className="material-symbols-outlined text-white/40">chevron_right</span>,
    },
  ];
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-white/60 mt-1">Manage your gym members and their subscriptions</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <StatsCard title="Total Members" value={stats?.totalMembers || 0} icon="group" color="primary" />
        <StatsCard title="Active" value={stats?.activeMembers || 0} icon="verified" color="green" />
        <StatsCard title="Inactive" value={stats?.inactiveMembers || 0} icon="event_busy" color="red" />
        <StatsCard title="Frozen" value={stats?.frozenMembers || 0} icon="ac_unit" color="orange" />
        <StatsCard title="Pending" value={stats?.pendingMembers || 0} icon="schedule" color="blue" />
      </div>
      ...
      <div>
        <DataTable<Member>
          ...
          onRowClick={(member) => router.push(`/${locale}/admin/members/${member.id}`)}
        />
        {members && (
          <Pagination
