  ...
  const columns: Column<Attendance>[] = [
    {
      ...
      render: (record: Attendance) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-medium">{record.member?.fullName || 'Unknown'}</p>
            <p className="text-white/40 text-xs">{record.member?.memberId || '-'}</p>
            <p className="text-white/50 text-xs">Service: {record.member?.service?.name || record.member?.serviceType || '-'}</p>
          </div>
        </div>
      ),
    },
    {
      key: 'date',
      header: 'Date',
      render: (record: Attendance) => <span className="text-white/80">{formatDate(record.date)}</span>,
    },
    {
      key: 'checkIn',
      header: 'Check-in',
    ...
    },
    {
      key: 'checkOut',
      header: 'Check-out',
      render: () => <span className="text-white/40">-</span>,
    },
    {
      key: 'duration',
      header: 'Duration',
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard title="Today's Check-ins" value={stats?.today || 0} icon="today" color="blue" />
        <StatsCard title="This Week" value={stats?.thisWeek || 0} icon="date_range" color="green" />
        <StatsCard title="This Month" value={stats?.thisMonth || 0} icon="calendar_month" color="orange" />
        <StatsCard title="Daily Average" value={stats?.averageDaily?.toFixed(1) || '0'} icon="analytics" color="primary" />
      </div>
            ...
            <>
              {today.checkIns.slice(0, 10).map((checkIn) => (
                <div key={checkIn.id} className="flex items-center gap-2 px-3 py-2 bg-surface-dark-lighter rounded-lg">
                  ...
                  <div>
                    <p className="text-white text-sm font-medium">{checkIn.memberName || 'Unknown'}</p>
      ...
      <div className="bg-surface-dark rounded-xl border border-surface-dark-lighter p-4">
        <form
          onSubmit={(e) => { e.preventDefault(); setPage(1); fetchAttendance(); }}
          className="flex flex-col sm:flex-row gap-4"
        >
          <div className="flex-1 relative">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-white/40">calendar_today</span>
            <input
              type="date"
              value={dateFilter}
