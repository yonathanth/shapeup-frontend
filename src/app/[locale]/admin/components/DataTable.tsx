            ...
            {data.length === 0 ? (
              ...
              data.map((item) => (
                <tr
                  ...
                  className={`transition-colors ${onRowClick ? 'cursor-pointer hover:bg-surface-dark-lighter' : ''}`}
                >
                  {columns.map((column) => (
                    <td key={column.key} className={`px-4 py-4 text-sm text-white/80 ${column.className || ''}`}>
                      {column.render
                        ? column.render(item)
...
export function Pagination({ currentPage, totalPages, onPageChange }: PaginationProps) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-t border-surface-dark-lighter">
      <div className="text-sm text-white/40">Page {currentPage} of {totalPages}</div>
      <div className="flex items-center gap-1">
        ...
        {startPage > 1 && (
          <>
            <button onClick={() => onPageChange(1)} className="px-3 py-1 rounded-lg text-sm text-white/60 hover:bg-surface-dark-lighter transition-colors">1</button>
        ...
        {pages.map((page) => (
          <button
            ...
            className={`px-3 py-1 rounded-lg text-sm transition-colors ${page === currentPage ? 'bg-primary text-white font-medium' : 'text-white/60 hover:bg-surface-dark-lighter'}`}
          >
            {page}
          </button>
        ))}
        {endPage < totalPages && (
          <>
            ...
            <button onClick={() => onPageChange(totalPages)} className="px-3 py-1 rounded-lg text-sm text-white/60 hover:bg-surface-dark-lighter transition-colors">{totalPages}</button>
          </>
        )}
        <button
          onClick={() => onPageChange(currentPage + 1)}
