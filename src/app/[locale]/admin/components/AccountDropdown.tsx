...
export default function AccountDropdown() {
  ...
  const [formData, setFormData] = useState({ email: '', password: '', currentPassword: '' });
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => { setShowUpdateForm(false); setUpdateSuccess(false); }, 2000);
    } catch (error: unknown) {
            ...
            <div className="p-4 border-t border-surface-dark-lighter">
              <form onSubmit={handleUpdateProfile} className="space-y-3">
                <div>
                  ...
                  <input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder={user?.email || 'Email'} className="w-full h-9 px-3 bg-surface-dark-lighter border border-surface-dark-lighter rounded-lg text-white text-sm placeholder:text-white/40 focus:outline-none focus:border-primary/50 transition-colors" />
                </div>
                <div>
                  <label className="block text-white/60 text-xs mb-1">New Password</label>
                  <input type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="Leave empty to keep current" className="w-full h-9 px-3 bg-surface-dark-lighter border border-surface-dark-lighter rounded-lg text-white text-sm placeholder:text-white/40 focus:outline-none focus:border-primary/50 transition-colors" />
                </div>
                <div>
                  <label className="block text-white/60 text-xs mb-1">Current Password</label>
                  <input type="password" value={formData.currentPassword} onChange={(e) => setFormData({ ...formData, currentPassword: e.target.value })} placeholder="Required for changes" required className="w-full h-9 px-3 bg-surface-dark-lighter border border-surface-dark-lighter rounded-lg text-white text-sm placeholder:text-white/40 focus:outline-none focus:border-primary/50 transition-colors" />
                </div>
