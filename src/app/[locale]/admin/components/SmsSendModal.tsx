...
export default function SmsSendModal({ isOpen, onClose, initialPhone = '', initialMemberId, onSuccess }: SmsSendModalProps) {
  ...
  useEffect(() => { setPhone(initialPhone); setMemberId(initialMemberId); }, [initialPhone, initialMemberId]);
  ...
  useEffect(() => {
    if (memberSearch.length > 2) {
      const timeoutId = setTimeout(async () => {
        ...
        } catch { console.error('Member search failed'); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(''); setPhone(''); setMemberId(undefined);
      onSuccess?.();
      onClose();
    } catch (err) {
      ...
      if (msg.includes('unverified contact') || msg.includes('verify')) {
        msg = 'This phone number is not verified. Please verify it in your SMS provider dashboard first.';
      }
      setError(msg);
      ...
      <div className="bg-surface-dark rounded-xl border border-surface-dark-lighter w-full max-w-md">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={onClose} className="text-white/60 hover:text-white" disabled={isSending}>
              <span className="material-symbols-outlined">close</span>
            </button>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              <label className="block text-white/80 text-sm mb-2">Search Member (optional)</label>
              <input type="text" value={memberSearch} onChange={(e) => setMemberSearch(e.target.value)} placeholder="Search by name or phone..." className="w-full px-4 py-2 bg-surface-dark-lighter border border-surface-dark-lighter rounded-lg text-white placeholder-white/40 focus:outline-none focus:border-primary" disabled={isSending} />
              {memberResults.length > 0 && (
                <div className="absolute z-10 w-full mt-1 bg-surface-dark border border-surface-dark-lighter rounded-lg overflow-hidden">
                  {memberResults.map((member) => (
                    <button key={member.id} type="button" onClick={() => handleMemberSelect(member)} className="w-full text-left px-4 py-2 hover:bg-surface-dark-lighter text-white">
            ...
            <div>
              <label className="block text-white/80 text-sm mb-2">Phone Number <span className="text-red-400">*</span></label>
              <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+251912345678" required className="w-full px-4 py-2 bg-surface-dark-lighter border border-surface-dark-lighter rounded-lg text-white placeholder-white/40 focus:outline-none focus:border-primary" disabled={isSending} />
            </div>
            <div>
              ...
              <div className="flex items-center justify-between mt-1">
                <div className="text-white/60 text-xs">{message.length} characters • {smsCount} SMS</div>
                <div className="text-white/60 text-xs">{1600 - message.length} remaining</div>
              </div>
            </div>
