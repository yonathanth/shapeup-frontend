      ...
      {isLoading && !balance ? (
        ...
        <div className="space-y-2">
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-white">{parseFloat(balance.balance).toFixed(2)}</span>
            <span className="text-white/60 text-sm">ETB</span>
          </div>
          <div className="text-white/60 text-sm">~{balance.estimatedMessages.toLocaleString()} messages available</div>
        </div>
      ) : null}
    </div>
  );
